...
from torchao.int8.float8_scaling_utils import (
...
from torchao.int8.float8_utils import e4m3_dtype, EPS
...
def precompute_float8_dynamic_scale_for_fsdp(module: nn.Module) -> None:
    ...
    from torchao.int8.config import ScalingType
    from torchao.int8.float8_linear import Float8Linear
...
class WeightWithDelayedFloat8CastTensor(torch.Tensor):
    ...
    def fsdp_pre_all_gather(self, mesh):
        ...
        if not self.is_amax_initialized:
            from torchao.int8.float8_linear import (
